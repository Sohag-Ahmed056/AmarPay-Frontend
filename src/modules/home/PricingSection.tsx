...
const Header = () => (
  <header className="relative text-center mb-12 md:mb-20 px-4 z-10">
    ...
    <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold text-gray-900 dark:text-white leading-tight tracking-tighter animate-fade-in-down">
      All Charges of <br />
      <span className="text-transparent bg-clip-text bg-gradient-to-r from-secondary to-primary"> Amar Pay</span>
    </h1>
    ...
    <p className="text-gray-600 dark:text-gray-300 mt-6 text-base sm:text-lg max-w-2xl mx-auto animate-fade-in-down" style={{animationDelay: '0.4s'}}>
        Want to know about all the charges of amar pay? Here is it.
    </p>
  </header>
);
  ...
  const pricingPlans = [
    {
      plan: 'Send Money',
      price: 14,
      description: 'A service fee of 14 BDT is applied per 1,000 BDT sent.',
      features: [
        'Instant money transfer to another user',
        'Applicable service charge on each transaction',
        'Recipient receives full credited amount',
      ],
      isFeatured: false,
    },
    {
      plan: 'Cash Out',
      price: 5,
      description: 'Withdraw cash from your wallet at an agent point for 5 BDT per 1,000 BDT.',
      features: [
        'Withdraw funds via authorized agents',
        'Agent receives digital confirmation',
        'User collects physical cash instantly',
      ],
      isFeatured: true,
    },
    {
      plan: 'ATM Withdrawal',
      price: 10,
      description: 'Withdraw money directly from ATMs for 10 BDT per 1,000 BDT.',
      features: [
        'Enter AmarPay PIN securely at ATM',
        'Receive cash immediately',
        'Service fee deducted automatically',
        'Track all withdrawals in transaction history',
      ],
      isFeatured: false,
    },
  ];
